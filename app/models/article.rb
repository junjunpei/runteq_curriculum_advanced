...
class Article < ApplicationRecord
  ...
  enum eyecatch_place: { left: 0, center: 1, right: 2 }
  ...
  validates_numericality_of :eyecatch_width, greater_than_or_equal_to: 100, less_than_or_equal_to: 700, only_integer: true, allow_nil: true
  ...
  scope :by_tag,      ->(tag_id) { Article.joins(:article_tags).where(article_tags: { tag_id: tag_id }) }
  scope :title_contain, ->(word) { where('title LIKE ?', "%#{word}%") }
  scope :body_contain,  ->(word) { Article.joins(:sentences).where('sentences.body LIKE ?', "%#{word}%") }
